...
$(document).ready(function () {
  ...
  /*
  function getItems() {
    $.get("http://www.mocky.io/v2/5cdffe2d330000ff11608b6e", function (data) {

      for (var i = 0; i < data.length; i++) {
        var item = data[i];
        item.time = parseInt(Math.random() * 100);
        itemsDiv = $("#" + modelTypesIds[modelTypesNum.indexOf(3)] + "-wrapper");
        var modelformat = (item.format) ? ' model-format="' + item.format + '"' : "";
        var html = '<div class="col-sm-4">' + '<a class="thumbnail add-item"' +' model-name="'+ item.name +'"' +' model-url="' +item.model+'"' +' model-type="' +item.type+'"' + modelformat+'>'+'<img src="'+item.image +'" alt="Add Item"  data-dismiss="modal"> '+item.name + '[' + item.time + ']' + +'</a></div>';
